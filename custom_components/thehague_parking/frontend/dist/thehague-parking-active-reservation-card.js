...
// src/thehague-parking-active-reservation-card-editor.ts
var TheHagueParkingActiveReservationCardEditor = class extends i4 {
  constructor() {
    super(...arguments);
    this._entries = [];
  ...
  _applyEntry(entryId) {
    ...
    const entity = slug ? `sensor.thehague_parking_${slug}_reservations` : void 0;
    this._configChanged({
      ...this._config,
...
TheHagueParkingActiveReservationCardEditor.styles = i`
    .container {
      display: flex;
...
__decorateClass([
  n4({ attribute: false })
], TheHagueParkingActiveReservationCardEditor.prototype, "hass", 2);
__decorateClass([
  n4({ attribute: false })
], TheHagueParkingActiveReservationCardEditor.prototype, "_config", 2);
__decorateClass([
  r5()
], TheHagueParkingActiveReservationCardEditor.prototype, "_entries", 2);
__decorateClass([
  r5()
], TheHagueParkingActiveReservationCardEditor.prototype, "_entriesLoaded", 2);
TheHagueParkingActiveReservationCardEditor = __decorateClass([
  t3("thehague-parking-active-reservation-card-editor")
], TheHagueParkingActiveReservationCardEditor);

// src/thehague-parking-active-reservation-card.ts
var TheHagueParkingCard = class extends i4 {
  constructor() {
    super(...arguments);
  ...
  get _reservationsEntityId() {
    if (!this._config) return void 0;
    const override = (this._config.entity ?? "").trim();
    if (override) return override;
    const slug = this._slug;
    return slug ? `sensor.thehague_parking_${slugifyId(slug)}_reservations` : void 0;
  }
  get _entity() {
    const entityId = this._reservationsEntityId;
    return this.hass && entityId ? this.hass.states[entityId] : void 0;
  }
  get _reservations() {
    const reservations = this._entity?.attributes?.reservations;
    return Array.isArray(reservations) ? reservations : void 0;
  }
  _notify(message) {
    this.dispatchEvent(
  ...
  _reservationLabel(reservation) {
    ...
    return (name && plate ? `${name} - ${plate}` : name || plate) || localize(this.hass, "active_reservation_card.reservation_fallback_label");
  }
  _formatTime(value) {
    if (!value) return;
    const date = new Date(value);
    return Number.isNaN(date.getTime()) ? void 0 : date.toLocaleTimeString(void 0, {
      hour: "2-digit",
      minute: "2-digit"
    });
  }
  ...
  render() {
    ...
    if (!this._reservationsEntityId) {
      const title2 = this._config.title ?? localize(this.hass, "active_reservation_card.default_title");
      if (this._config.config_entry_id && this._resolvingService) {
        return x`
          <ha-card header=${title2}>
    ...
    return x`
      <ha-card header=${title}>
        <div class="card-content">
          ${reservations.length === 0 ? x`<div class="empty">
      ...
      return x`
                      <div class="row">
                        <div class="main">
                          <div class="label">${this._reservationLabel(r6)}</div>
                        ...
                        <div class="actions">
                          ...
                          >
                            ${ending ? localize(this.hass, "common.working") : localize(this.hass, "active_reservation_card.end")}
                          </ha-button>
                        </div>
                      </div>
                    `;
...
__decorateClass([
  n4({ attribute: false })
], TheHagueParkingCard.prototype, "hass", 2);
__decorateClass([
  r5()
], TheHagueParkingCard.prototype, "_config", 2);
__decorateClass([
  r5()
...
__decorateClass([
  r5()
], TheHagueParkingCard.prototype, "_resolvedConfigEntryId", 2);
__decorateClass([
  r5()
], TheHagueParkingCard.prototype, "_resolvedMeldnummer", 2);
TheHagueParkingCard = __decorateClass([
  t3("thehague-parking-card")
], TheHagueParkingCard);
window.customCards ??= [];
